var dataTesting = {
    ...
    'data.float64.le.dat': { type: Float64Array, values: [ 0, 512.1024, 2048.4096, 8192.16384, 32768.65535]},
...
describe("Tonic Data Manager - Download Binary Data", function() {
        it('test all data format', function(done) {
            ...
            function processData(name) {
                tonicDataManager.on('/base/data/binary/' + name, function(data, envelope) {
                    testHelper(null, data);
                });

                // Trigger the download request
                var fetchURL = '/base/data/binary/' + name,
                    url = tonicDataManager.fetchURL(fetchURL, 'arraybuffer');
